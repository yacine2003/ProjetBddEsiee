...
public class CommandeDAO {
    ...
    public boolean inserer(Commande commande) {
        ...
        for (DetailCommande detail : commande.getDetailsCommande()) {
            montantTotal += detail.getPrixUnitaire() * detail.getQuantite();
        }
        // Vérification du solde via la procédure stockée
        try (Connection conn = DatabaseConfig.getConnection()) {
            CallableStatement cs = conn.prepareCall("{call verifier_solde(?, ?, ?)}");
            cs.setInt(1, commande.getClient().getIdClient());
            cs.setDouble(2, montantTotal);
            cs.registerOutParameter(3, java.sql.Types.BOOLEAN);
            cs.execute();
            soldeSuffisant = cs.getBoolean(3);
        } catch (SQLException e) {
            System.err.println("Erreur lors de la vérification du solde : " + e.getMessage());
            return false;
        }
        if (!soldeSuffisant) {
            System.err.println("Solde insuffisant pour le client : " + commande.getClient().getIdClient());
            return false;
        }
            ...
            if (affectedRows > 0) {
                try (ResultSet rs = pstmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        ...
                        // Appel de la procédure de facturation
                        try (CallableStatement csFacture = conn.prepareCall("{call facturer_commande(?)}")) {
                            csFacture.setInt(1, commande.getIdCommande());
                            csFacture.execute();
                        } catch (SQLException e) {
                            System.err.println("Erreur lors de la facturation : " + e.getMessage());
                        }
                        return true;
                    }
                }
